...
**Note on Persistence**: In the digital hardware domain, signals represent physical wires that hold their voltage level until driven to a new value. Therefore, ports are defined as `PERSISTENT`. This ensures that `read()` method on an input port returns the last known value even if the signal wasn't updated in the current simulation step, mimicking the behavior of a hard-wire which is persistent by nature. Similarly, an output port that hasn't been explicitly written on through the `write()` method is kept updated regardless, every time with the value set in the previous simulation step.
